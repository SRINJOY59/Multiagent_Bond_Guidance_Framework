...
class CompanyCashflow:
    ...
    def _load_and_validate_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            required_columns = [
                'id', 'isin', 'cash_flow_date', 'cash_flow_amount',
                'record_date', 'principal_amount', 'interest_amount',
                'tds_amount', 'remaining_principal', 'state'
            ]
            
            missing_cols = [col for col in required_columns if col not in df.columns]
            if missing_cols:
                raise CashFlowValidationError(f"Missing required columns: {missing_cols}")
    ...
    def _get_isin_details(self, isin: str) -> Dict[str, Any]:
        ...
        details = {
            ...
            'next_payment_date': isin_data[isin_data['cash_flow_date'] > datetime.now()]['cash_flow_date'].min(),
            'maturity_date': isin_data['cash_flow_date'].max(),
            'payment_schedule': isin_data[['cash_flow_date', 'cash_flow_amount', 'principal_amount', 'interest_amount']]
    ...
    def query(self, input_query: str) -> str:
        """
        Process user queries with enhanced error handling and response formatting
        
        Args:
            input_query (str): User's query string
            
        Returns:
            str: Formatted response to the query
        """
        try:
            # Check for ISIN-specific queries
            if "ISIN" in input_query.upper():
                isin = input_query.upper().split("ISIN")[-1].strip()
                try:
                    details = self._get_isin_details(isin)
                    return self._format_isin_response(details)
                except ISINNotFoundError:
                    return f"No data found for ISIN: {isin}"
            
            # Process through the agent for other queries
    ...
    def _format_isin_response(self, details: Dict[str, Any]) -> str:
        ...
        next_payment = details['next_payment_date'].strftime('%Y-%m-%d') if details['next_payment_date'] else "No upcoming payments"
...
"""
    ...
    def get_upcoming_maturities(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
